 ...
 * 
 * The main difference between oscillators is that the first one (DSF2) generates
 * one-sided spectrum, while the second (DSF4) generates spectrum with equal number
 * of partials on both sides.
 * 
 * Param A sets tuning, buttons choose submenu
 * 
 * Main menu:
 * - param B morphs output of both oscillators
 * - param C - alpha parameter for DSF, this sets slope for amplitude of generated
 * partials
 * - param D - beta parameter for DSF, sets distance between partials. This allows
 * createing harmonic or inharmonic waveforms
 * 
 * Submenu 1 controls phase modulation
 * - param B sets PM amount
 * - param C/D set ratio for modulator. C is used as multiplier with integer values
 * in [1..5] range, D is the divider in [0..5] range. If D is set to 0 then C is
 * used as unquantized values in [0.2..5] range with exponential growth
 * 
 * Submenu 2 controls oscillator feedback and VCA/wavefolder
 * - param B sets gain for wavefolder in [0..4] range. Using lower values allow using
 * it as a simple end of chain VCA
 * - param C sets direction (polar phase) for primary oscillator's feedback. This
 * determines direction where complex waveform is stretched.
 * - param D adds feedback to complex oscillator. This stretches complex oscillator shape
 * towards one of the corners.
 * 
 * Parameters F and G can be used as outputs for modulation
 * 
 * Audio input 1 is V/Oct pitch CV, input 2 is used as a source for external PM. Pitch
 * CV can be quantized if you rebuild patch with quantizer enabled.
 **/
