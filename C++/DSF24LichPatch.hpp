#ifndef __DSF_2_4_LICH_PATCH_HPP__
#define __DSF_2_4_LICH_PATCH_HPP__

#include "DiscreteSummationOscillator.hpp"
#include "MultiOscillator.hpp"
#include "QuadratureMorphingOscillator.hpp"
...
public:
    ModOscillator(float freq, float sr)
    ...
    static ModOscillator* create(float freq, float sr) {
        return new ModOscillator(freq, sr);
    }

    static void destroy(ModOscillator* osc) {
        delete osc;
    }
...
using MorphBase =
    QuadratureMorphingOscillator<DiscreteSummationOscillator<DSF2>, DiscreteSummationOscillator<DSF4>>;
...
class DSF24LichPatch : public Patch {
private:
    SineOscillator* mod;
...
public:
    DSF24LichPatch() {
        ...
        setParameterValue(P_FM_MULT, 0.0);
        registerParameter(P_FM_DIV, "FM Divider");
        setParameterValue(P_FM_DIV, 0.0);
...
// Carrier / modulator frequency
        ...
        float fm_amount = p_fm_amount.getValue() * MAX_FM_AMOUNT;
        ...
        int divisor = (MAX_RATIO + 1) * p_fm_div.getValue();
        if (divisor == 0) {
            mod->setFrequency(freq * p_fm_ratio.getValue());
        }
        else {
            int multiplier = MAX_RATIO * p_fm_mult.getValue() + 1;
            mod->setFrequency(freq * float(multiplier) / float(divisor));
        }
