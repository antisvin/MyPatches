...
public:
    enum Parameters { PARAMETER_X, PARAMETER_Y, PARAMETER_ENV };
    MorphSynth(MorphOsc* osc, AdsrEnvelope* env)
        : osc(osc)
        , env(env) {
    }
...
public:
    MorphMonoGenerator(MorphOsc* osc, AdsrEnvelope* env)
        : MorphSynth(osc, env) {
    }
    float generate() {
...
public:
    MorphStereoGenerator(
        MorphOsc* left, MorphOsc* right, AdsrEnvelope* env, FloatArray buffer)
        : MorphSynth(left, env)
        , osc2(right)
        , buffer(buffer) {
    }
    ...
    static MorphStereoGenerator* create(
        MorphBank* bank1, MorphBank* bank2, float sr, size_t bs) {
